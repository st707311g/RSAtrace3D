...
class QtMenubar(QMenuBar):
    ...
    def build(self):
        ...
        self.menu_history = self.history.build_menu(parent=self, triggered=self.on_menu_history)
        self.menu_file.addMenu(self.menu_history)

        self.act_close_volume = QtAction(
            text='Close volume', parent=self, shortcut='Ctrl+D',
            statusTip='Close volume', triggered=self.on_act_close_volume, 
            auto_enable_volume=True)
        self.menu_file.addAction(self.act_close_volume)
        self.menu_file.addSeparator()

        self.act_save_rinfo = QtAction(
            text='Save rinfo file', parent=self, shortcut='Ctrl+S',
            statusTip='Save rinfo file', triggered=self.on_act_save_rinfo,
            auto_enable_volume=True)
        self.menu_file.addAction(self.act_save_rinfo)

        self.act_export_projections = QtAction(
            text='Export projection images', parent=self, shortcut='Ctrl+P',
            statusTip='Export projection images', triggered=self.on_act_export_projections,
            auto_enable_volume=True)
        self.menu_file.addAction(self.act_export_projections)

        self.act_export_trace_images = QtAction(
            text='Export trace images', parent=self, shortcut='Ctrl+T', 
            statusTip='Export trace images (Z-stack image)', triggered=self.on_act_export_trace_images,
            auto_enable_volume=True)
        self.menu_file.addAction(self.act_export_trace_images)

        self.act_export_root_csv = QtAction(
            text='Export root traits (csv)', shortcut='Ctrl+E', 
            statusTip='Export root traits (csv)', triggered=self.on_act_export_root_csv,
            auto_enable_volume=True)
        ...
        self.menu_interpolation = interpolation.build_menu(menu_label='&Interpolation', triggered=self.on_menu_interpolation)
        ...
        #// extension menu
        self.menu_extensions = self.parent().extensions.build_menu(menu_label='&Extensions', triggered=self.on_menu_extensions)
        ...
        self.act_about = QtAction(
            text='About', parent=self.parent(),
            statusTip='About RSAtrace3D', triggered=self.on_act_about)
    ...
    def on_act_export_root_csv(self):
        try:
            ...
            with open(csv_fname, 'w', newline="") as f:
                ...
                f.write(f'# Volume name: {volume_name}, Resolution: {resolution}\n')
                df.to_csv(f)
