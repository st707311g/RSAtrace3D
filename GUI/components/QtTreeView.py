    ...
    def get_iter_all_items(self, column, parent=None):
        ...
        if parent is None:
            for row in range(self.rowCount()):
                ...
                yield from self.get_iter_all_items(column=column, parent=self.item(row, 0))
        else:
            for row in range(parent.rowCount()):
                yield parent.child(row, column)
                yield from self.get_iter_all_items(column=column, parent=parent.child(row, 0))
...
class QtTreeView(QTreeView):
    ...
    def __init__(self, parent):
        ...
        self.model.setHorizontalHeaderLabels(self.root_traits.class_container.labels())
    ...
    def create_row(self, ID_string: ID_Object):
        ...
        for class_ in self.root_traits.class_container:
            ret = class_(self.RSA_components().vector, ID_string).QStandardItem()
            row.append(ret)

        return row

    def add_base(self, ID_string: ID_Object, annotations={}, auto_selection=True):
    ...
    def add_root(self, ID_string: ID_Object, annotations={}, auto_selection=True):
        base_index = self.model.get_selected_base_index(selection_model=self.selectionModel())
    ...
    def add_relay(self, ID_string: ID_Object, annotations={}, auto_selection=True):
    ...
    def get_selected_ID_string(self):
        ...
        return None if item is None else ID_Object(self.model.itemFromIndex(index).text())
    ...
    def selectionChanged(self, selected, deselected):
        ...
        if not self.parent().is_control_locked():
            ...
            self.GUI_components().projectionview.on_selected_item_changed(ID_string=ID_string)
