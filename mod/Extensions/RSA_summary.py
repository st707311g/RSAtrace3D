...
from PyQt5.QtWidgets import QApplication, QMainWindow, QTableView, QHeaderView, QAction, QFileDialog, QMenuBar, QAction, QMenu, QStatusBar, QProgressBar, QLabel, QSizePolicy

import pandas as pd
from typing import List

from .__backbone__ import ExtensionBackbone
from DATA import RSA_Vector, RinfoFiles
...
class SummaryTableview(QTableView):
    def __init__(self, parent):
        ...
        self.model.setHorizontalHeaderLabels(self.RSA_traits().class_container.labels())

        for ci, cls_ in enumerate(self.RSA_traits().class_container):
            item = self.model.horizontalHeaderItem(ci)
            try: item.setToolTip(cls_.tool_tip)
            except: pass
...
class QtMenubar(QMenuBar):
    ...
    def build(self):
        ...
        self.act_import_rinfo = QAction(
            text='Import rinfo file(s)', parent=self.parent(), shortcut='Ctrl+O', 
        ...
        self.act_import_rinfo_from_directory = QAction(
            text='Import rinfo file(s) from a directory', parent=self.parent(), shortcut='Ctrl+Shift+O', 
        ...
        self.act_edit_remove_row = QAction(
            text='Remove selected row(s)', parent=self.parent(), shortcut='Delete',
            triggered=self.on_act_edit_remove_selected)
        self.menu_edit.addAction(self.act_edit_remove_row)

        self.act_edit_clear = QAction(
            text='Remove all rows', parent=self.parent(), shortcut='Ctrl+Delete',
    ...
    def on_act_import_rinfo_file(self):
        files = QFileDialog.getOpenFileNames(self, 'Import rinfo file(s)', '', 'rinfo Files (*.rinfo)')[0]
        if len(files)==0:
            return
    ...
    def on_act_import_rinfo_from_directory(self):
        directory = QFileDialog.getExistingDirectory(self, 'Volume directory', '')
        if directory == '':
            return False
    ...
    def on_act_export_root_csv(self):
        csv_fname = QFileDialog.getSaveFileName(None, 'Save file', '', '*.csv')[0]
        if csv_fname:
            try:
