...
class COG_tracking(InterpolationBackbone):
    ...
    def interpolate(self, polyline: List[List[int]]):
        ...
        # // preparing
        self.func_get_subvolume = (
            self.RSA_components().volume.get_trimmed_volume
        )  # // a function returning a local subvolume
    ...
    def get_vector(self, from_, to_, prev_v):
        ...
        root_v = (
            (root_v / np.linalg.norm(root_v) + prev_v / np.linalg.norm(prev_v))
            / 2
            * self.unit_length
        )
        ...
        if sum_ == 0:
            ...
            cog_v = [
                np.sum(np.sum(indices * subvol, axis=axis) / np.sum(subvol))
                for indices, axis in zip(np.indices(subvol.shape), [2, 1, 0])
            ]
    ...
    def move_particle(self, start_pos, end_pos):
        self.velocity = self.get_vector(
            from_=start_pos, to_=end_pos, prev_v=self.velocity
        )
