...
class ModuleTest(object):
    def __init__(self, module_class=None):
        ...
        rinfo_files = [f for f in os.listdir(rinfo_dir) if f.lower().endswith('.rinfo')]
        rinfo_files.sort()
    ...
    def apply(self, module_class):
        ...
        for rsa_vector in self.rsa_vector_list:
            print(f'calculation results: {rsa_vector.annotations.volume_name()}')
            if module_class.class_type == 'root':
                for ID_string in rsa_vector.iter_all():
                    ins = module_class(RSA_vector=rsa_vector, ID_string=ID_string)
